import 'package:flutter/material.dart';

Color mainBlue = Color(0xff1F8DF5);
Color whiteColor = Color(0xffFFFFFF);
Color secondaryBlue = Color(0xff9ED0FF);
Color textColor = Color(0xff2C3A59);
Color secondaryTextColor = Color(0xff808BA2);
Color backgroundChat = Color(0xff505C6B);
Color greenColor = Color(0xff29CB9E);

TextStyle nameText =
    TextStyle(color: whiteColor, fontSize: 20, fontWeight: FontWeight.w400);

TextStyle secondaryNameText =
    TextStyle(color: secondaryBlue, fontSize: 16, fontWeight: FontWeight.w300);

TextStyle mainTitleTetxt =
    TextStyle(color: textColor, fontSize: 16, fontWeight: FontWeight.w600);

TextStyle secondaryBlackTetxt =
    TextStyle(color: textColor, fontSize: 14, fontWeight: FontWeight.w500);


TextStyle secondaryGreytxt =
    TextStyle(color: secondaryTextColor, fontSize: 14, fontWeight: FontWeight.w300);
